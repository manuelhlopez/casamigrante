...
public class TipoActividadModalDialog implements EventHandler<ActionEvent> {
	...
	public TipoActividadModalDialog(Stage primaryStage, CasosPrevencionAppPane appPane) {
	...
	public Stage buildModalDialogStage() {
		// Build the images...
		Image okImage = new Image(TipoActividadModalDialog.class.getResourceAsStream("/com/agt/client/images/ok.png"));
		ImageView okImageView = new ImageView(okImage);
		
		Image closeImage = new Image(TipoActividadModalDialog.class.getResourceAsStream("/com/agt/client/images/close.png"));
		...
		ArrayList<String> tipoActividadList = DashboardGlobalInfo.getTipoActividadList();
		...
		if (eventSource == this.okButton) {
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					if(selected.toLowerCase().startsWith("otra")){
						appPane.getTipoActividadTextField().setEditable(true);
						appPane.getTipoActividadTextField().setText("");
					}else{
						appPane.getTipoActividadTextField().setEditable(false);
						appPane.getTipoActividadTextField().setText(selected);
					}
