...
public class SituacionesSocialesModalDialog implements EventHandler<ActionEvent> {
	private Stage primaryStage;
	private Stage modalDialogStage;
	private Button okButton;
	private Button cancelButton;
	private CasosDesproteccionSocialAppPane appPane;
	private ArrayList<CheckBox> checkBoxList;
	
	public SituacionesSocialesModalDialog(Stage primaryStage, CasosDesproteccionSocialAppPane appPane) {
	...
	public Stage buildModalDialogStage() {
		// Build the images...
		Image okImage = new Image(SituacionesSocialesModalDialog.class.getResourceAsStream("/com/agt/client/images/ok.png"));
		ImageView okImageView = new ImageView(okImage);
		
		Image closeImage = new Image(SituacionesSocialesModalDialog.class.getResourceAsStream("/com/agt/client/images/close.png"));
	...
	public boolean findSelected(String id){
		for(CatalogInfo info : appPane.getSituacionDesproteccionInfoList()){
	...
	public void handle(ActionEvent event) {
		...
		if (eventSource == this.okButton) {
			if(this.checkBoxList != null){
				appPane.getSituacionDesproteccionInfoList().clear();
				for(CheckBox checkBox : this.checkBoxList){
					if(checkBox.isSelected()){
						CatalogInfo info = (CatalogInfo) checkBox.getUserData();
						appPane.getSituacionDesproteccionInfoList().add(info);
					}
				}
