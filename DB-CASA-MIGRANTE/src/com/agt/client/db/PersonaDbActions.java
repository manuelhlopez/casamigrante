 ...
 *
 */
public class PersonaDbActions {

	/*
	 * Método útil para guardar o editar un nuevo caso de desprotección...
	 */
	public boolean save(PersonaInfo personaInfo){
		try {
			...
			EntityStore store = new EntityStore(env, "personas", storeConfig);
			PrimaryIndex<String,PersonaInfo> casoId =
					store.getPrimaryIndex(String.class, PersonaInfo.class);

			Transaction txn = env.beginTransaction(null, null);

			if(personaInfo != null){
				casoId.put(personaInfo);
			}
			txn.commit();
			store.close();
			env.close();
	...
	// Método útil para obtener la lista de casos del año en curso...
		public ArrayList<PersonaInfo> getAll(){
			ArrayList<PersonaInfo> casosDesproteccionInfoList = null;
			/* Open a transactional Berkeley DB engine environment. */
			EnvironmentConfig envConfig = new EnvironmentConfig();
			envConfig.setAllowCreate(true);
			envConfig.setTransactional(true);
			...
			casoId = store.getPrimaryIndex(String.class, PersonaInfo.class);

			EntityCursor<PersonaInfo> empCursor = casoId.entities();
			try {

				for (PersonaInfo emp : empCursor) {
					if(casosDesproteccionInfoList == null){
						casosDesproteccionInfoList = new ArrayList<PersonaInfo>();
						casosDesproteccionInfoList.add(emp);
					}else{
						casosDesproteccionInfoList.add(emp);
					}
